Feature: Start and stop Neo4j Server
  ...
  Scenario: Start Neo4j Server and query Spatial layers
    ...
    Then sending JSON "{"query":"start n=node:test('bbox:[15.0, 16.0, 56.0, 61.0]') return n"}" to "http://localhost:7474/db/data/ext/CypherPlugin/graphdb/execute_query" should contain "node"
    Then sending JSON "{"script":"g.idx('test').get('bbox','[15.0, 16.0, 56.0, 61.0]')"}" to "http://localhost:7474/db/data/ext/GremlinPlugin/graphdb/execute_script" should contain "node"
