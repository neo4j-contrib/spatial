...
public class EnvelopeTests {
	...
	private static void testOverlaps(Envelope left, Envelope right, boolean intersects, double overlap,
		...
		assertThat("Expected bbox area", left.bbox(right).getArea(), closeTo(bboxArea, 0.000001));
		assertThat("Expected bbox area", right.bbox(left).getArea(), closeTo(bboxArea, 0.000001));
	}

	@Test
	public void shouldHandleIntersectionsIn2D() {
		Envelope left = new Envelope(0.0, 1.0, 0.0, 1.0);
		testOverlaps(left, new Envelope(0.0, 1.0, 0.0, 1.0), true, 1.0, 1.0, 1.0);        // copies
		testOverlaps(left, new Envelope(0.5, 1.0, 0.5, 1.0), true, 1.0, 0.25, 1.0);       // top right quadrant
		testOverlaps(left, new Envelope(0.25, 0.75, 0.25, 0.75), true, 1.0, 0.25, 1.0);   // centered
		testOverlaps(left, new Envelope(-0.5, 0.5, -0.5, 0.5), true, 0.25, 0.25,
				2.25);   // overlaps bottom left quadrant
		testOverlaps(left, new Envelope(-0.5, 1.5, -0.5, 1.5), true, 1.0, 1.0, 4.0);      // encapsulates
		testOverlaps(left, new Envelope(-1.0, 0.0, 0.0, 1.0), true, 0.0, 0.0, 2.0);       // touches left edge
		testOverlaps(left, new Envelope(0.5, 1.5, 1.0, 2.0), true, 0.0, 0.0, 3.0);        // touches top-right edge
	...
	private static void makeAndTestEnvelope(double[] min, double[] max, double[] width) {
		...
		for (int i = 0; i < min.length; i++) {
			...
			assertThat("Expected intersected width[" + i + "] to be correct", intersection.getWidth(i),
					equalTo(width[i]));
			area *= width[i];
		}
