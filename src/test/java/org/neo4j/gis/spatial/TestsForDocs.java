...
public class TestsForDocs extends Neo4jTestCase {
    ...
    private void importMapOSM() throws Exception {
        ...
        BatchInserter batchInserter = new BatchInserterImpl(getNeoPath().getAbsolutePath());
        importer.importFile(batchInserter, "map.osm", false);
        batchInserter.shutdown();

        GraphDatabaseService db = new EmbeddedGraphDatabase(getNeoPath().getAbsolutePath());
        importer.reIndex(db);
        db.shutdown();
    ...
    public void testImportOSM() throws Exception {
        ...
        GraphDatabaseService database = new EmbeddedGraphDatabase(getNeoPath().getAbsolutePath());
        try {
