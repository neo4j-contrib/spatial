...
public class LayerMergeTest {
	...
	private void createOSMIndex(OSMDataset dataset, OSMDataset.LabelHasher labelHasher, String layerName, Label label,
			String propertyKey) {
	...
	private class OSMLayerConfig extends LayerConfig {
		...
		protected void postCreate(int layerNum) {
			try {
				...
				createOSMIndex(dataset, labelHasher, layerNames[layerNum], OSMModel.LABEL_RELATION,
						OSMModel.PROP_RELATION_ID);
				try (Transaction indexTx = graphDb.beginTx()) {
	...
	private void shouldMergeLayersWithGeometries(boolean identicalLayers, int numGeoms, int geomLength,
			BiFunction<Transaction, String, Layer> layerMaker) {
		shouldMergeLayersWithGeometries(identicalLayers, numGeoms, geomLength, layerMaker, layerMaker,
				this::postCreateNone, DEFAULT);
	}
	...
	private void shouldMergeOSMLayersWithGeometries(boolean identicalLayers, int numGeoms, int geomLength) {
		...
		shouldMergeLayersWithGeometries(identicalLayers, numGeoms, geomLength, layerMaker, layerMaker, postCreate,
				config);
	}

	@SuppressWarnings("SameParameterValue")
	private void shouldMergeLayersWithGeometries(boolean identicalLayers, int numGeoms, int geomLength,
			BiFunction<Transaction, String, Layer> mainMaker, BiFunction<Transaction, String, Layer> otherMaker) {
		shouldMergeLayersWithGeometries(identicalLayers, numGeoms, geomLength, mainMaker, otherMaker,
				this::postCreateNone, DEFAULT);
	}

	private void shouldMergeLayersWithGeometries(boolean identicalLayers, int numGeoms, int geomLength,
			BiFunction<Transaction, String, Layer> mainMaker, BiFunction<Transaction, String, Layer> otherMaker,
			BiConsumer<LayerConfig, Integer> postCreate, LayerConfig layerConfig) {
		double scale = 0.01;
		...
		for (int l = 0; l < layerConfig.layerNames.length; l++) {
			...
			inTx(tx -> {
				...
				assertThat(format("Expected layer %s to have %d geometries, but found %d", layerName, numGeoms, count),
						count == numGeoms);
			});
		}
		...
		for (int i = 1; i < layerConfig.layerNames.length; i++) {
			...
			inTx(tx -> {
				...
				System.out.printf("Before merge %s->%s there were %d geometries and after merge there were %d%n",
						fromName, mainName, before.size(), after.size());
				if (verbose) {
					before.debug("before");
					other.debug("other");
					after.debug("after");
				...
				long expected = identicalLayers ? 0
						: (main instanceof OSMLayer) ? (long) numGeoms * (geomLength + 1) : numGeoms;
				assertThat(format("Expected to merge %d geometries into %s from %s, but merged %d", numGeoms, mainName,
						fromName, merged), merged == expected);
			});
		}
		...
		for (int i = 0; i < layerConfig.layerNames.length; i++) {
			...
			inTx(tx -> {
				...
				if (count != expected) {
					if (expected != 0) {
						System.out.printf("Expected layer %s to have %d geometries, but found %d", layerName, expected,
								count);
				...
				assertThat(format("Expected layer %s to have %d geometries, but found %d", layerName, expected, count),
						count == expected);
			});
		}
		...
		public void validate(String layerName, ArrayList<Geometry> added) {
			for (Geometry expectedGeometry : added) {
				if (!contains(expectedGeometry)) {
					System.out.printf("\tFailed to find expected geometry '%s' from layer %s%n", expectedGeometry,
							layerName);
				}
			}
		}
