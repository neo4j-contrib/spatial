...
public abstract class LayerIndexTestBase {
    ...
    public void shouldCreateAndFindIndexViaLayer() {
        ...
        try (Transaction tx = graph.beginTx()) {
            assertThat("Should find the same index", index.getLayer().getName(), equalTo(spatial.getLayer(tx, "test").getName()));
            assertThat("Index should be of right type", spatial.getLayer(tx, "test").getIndex().getClass(), equalTo(getIndexClass()));
        }
    }
    ...
    public void shouldFindNodeAddedDirectlyToIndex() {
        ...
        try (Transaction tx = graph.beginTx()) {
            SearchResults results = index.searchIndex(tx, new SearchIntersectWindow(((LayerIndexReader) index).getLayer(), new Envelope(0.0, 2.0, 0.0, 2.0)));
