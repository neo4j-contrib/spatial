...
public class TestDynamicLayers extends Neo4jTestCase implements Constants {
    ...
    private void runDynamicShapefile(String shpFile) throws Exception {
        ...
        try (Transaction tx = graphDb().beginTx()) {
            ...
            layers.add(shpLayer.addLayerConfig(tx, "CQL2-highway", GTYPE_LINESTRING, "highway is not null and geometryType(the_geom) = 'MultiLineString'"));
            layers.add(shpLayer.addLayerConfig(tx, "CQL3-residential", GTYPE_MULTILINESTRING, "highway = 'residential'"));
            layers.add(shpLayer.addLayerConfig(tx, "CQL4-nameV", GTYPE_LINESTRING, "name is not null and name like 'V%'"));
            layers.add(shpLayer.addLayerConfig(tx, "CQL5-nameS", GTYPE_LINESTRING, "name is not null and name like 'S%'"));
            layers.add(shpLayer.addLayerConfig(tx, "CQL6-nameABC", GTYPE_LINESTRING, "name like 'A%' or name like 'B%' or name like 'B%'"));
    ...
    private void runShapefileExport(String osmFile) throws Exception {
        ...
        try (Transaction tx = graphDb().beginTx()) {
            ...
            layers.add(osmLayer.addLayerConfig(tx, "CQL1-highway", GTYPE_LINESTRING, "highway is not null and geometryType(the_geom) = 'LineString'"));
            layers.add(osmLayer.addLayerConfig(tx, "CQL2-residential", GTYPE_LINESTRING, "highway = 'residential' and geometryType(the_geom) = 'LineString'"));
            layers.add(osmLayer.addLayerConfig(tx, "CQL3-natural", GTYPE_POLYGON, "natural is not null and geometryType(the_geom) = 'Polygon'"));
            layers.add(osmLayer.addLayerConfig(tx, "CQL4-water", GTYPE_POLYGON, "natural = 'water' and geometryType(the_geom) = 'Polygon'"));
            layers.add(osmLayer.addLayerConfig(tx, "CQL5-bbox", GTYPE_GEOMETRY, "BBOX(the_geom, " + toCoordinateText(bbox) + ")"));
