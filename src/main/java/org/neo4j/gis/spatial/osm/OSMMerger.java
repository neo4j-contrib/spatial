...
import static org.neo4j.gis.spatial.osm.OSMModel.PROP_BBOX;
...
public class OSMMerger {
	...
	private boolean checkNodeMoved(Node thisNode, long node_osm_id, String expectedPropKey, String nodePropKey,
			String keyDescription, Object value) {
		if (expectedPropKey.equals(nodePropKey)) {
			...
			if (Math.abs(thisValue - thatValue) > 0.00000001) {
				System.out.printf("Node '%d' has moved %s from %f to %f%n", node_osm_id, keyDescription, thisValue,
						thatValue);
				return true;
			}
		}
	...
	private static class MergeStats {
		...
		void printStats() {
			...
			if (countMoved > 0) {
				System.out.printf("During merge %d out of %d existing %s were moved - re-indexing required%n",
						countMoved, countReplaced, name);
			}
			if (countAdded > 0) {
				System.out.printf("During merge %d %s were added - re-indexing required%n", countAdded, name);
			}
			if (geomNodesToAdd.size() > 0) {
				System.out.printf("During merge %d geometry %s were identified for use in re-indexing%n",
						geomNodesToAdd.size(), name);
			}
		}
	}
			...
			if (node_osm_id != INVALID_OSM_ID) {
				...
				if (thisNode != null) {
					...
					for (String nodePropKey : node.getPropertyKeys()) {
						...
						moved = moved || checkNodeMoved(thisNode, node_osm_id, PROP_NODE_LON, nodePropKey, "longitude",
								value);
						moved = moved || checkNodeMoved(thisNode, node_osm_id, PROP_NODE_LAT, nodePropKey, "latitude",
								value);
	...
	private MergeStats mergeRelations(Transaction tx, OSMDataset dataset, OSMDataset other) {
		...
		Label thisLabelHash = OSMDataset.hashedLabelFrom(thisIndex.getName());
		Label thatLabelHash = OSMDataset.hashedLabelFrom(thatIndex.getName());
		return mergeOSMEntities(tx, "relations", PROP_RELATION_ID, thisLabelHash, thatLabelHash,
				this::relationOverlapMerger);
	}
	...
	private MergeStats mergeOSMEntities(Transaction tx, String entityName, String propertyKey, Label thisLabelHash,
			Label thatLabelHash, BiConsumer<Node, Node> overlapMerger) {
				...
				if (osm_id != INVALID_OSM_ID) {
					...
					if (thisEntityNode != null) {
						...
						if (thisEnvelope == null && thatEnvelope == null) {
							System.out.printf(
									"While merging OSM %s, found nodes with %s = %d which have no geometry on either original or merge nodes%n",
									entityName, propertyKey, osm_id);
						} else if (thisEnvelope == null) {
							System.out.printf(
									"While merging OSM %s, found nodes with %s = %d where the original has no geometry, but the merge node has a geometry%n",
									entityName, propertyKey, osm_id);
							stats.countMoved++;
							overlapMerger.accept(thisEntityNode, thatEntityNode);
						} else if (thatEnvelope == null) {
							System.out.printf(
									"While merging OSM %s, found nodes with %s = %d where the original has a geometry, but the merge node does not%n",
									entityName, propertyKey, osm_id);
					...
					System.out.printf("Unexpectedly found OSM %s at %s without property: %s%n", entityName,
							thatEntityNode, propertyKey);
				}
			}
			stats.printStats();
			return stats;
