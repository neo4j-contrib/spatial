...
import org.neo4j.gis.spatial.pipes.filtering.*;
import org.neo4j.gis.spatial.pipes.impl.*;
import org.neo4j.gis.spatial.pipes.processing.*;
...
public class GeoPipeline extends Pipeline<GeoPipeFlow, GeoPipeFlow> {
     ...
     */
    public static GeoPipeline startEqualExactSearch(final Transaction tx, Layer layer, Geometry geometry, double tolerance) {
     ...
     * @return geoPipeline
     */
    public static GeoPipeline startNearestNeighborLatLonSearch(final Transaction tx, Layer layer, Coordinate point, int numberOfItemsToFind) {
        Envelope searchWindow = SpatialTopologyUtils.createEnvelopeForGeometryDensityEstimate(tx, layer, point, numberOfItemsToFind);
     ...
     * @return geoPipeline
     */
    public static GeoPipeline startNearestNeighborLatLonSearch(final Transaction tx, Layer layer, Coordinate point, Envelope searchWindow) {
     ...
     * @return geoPipeline
     */
    public static GeoPipeline startNearestNeighborLatLonSearch(final Transaction tx, Layer layer, Coordinate point, double maxDistanceInKm) {
        Envelope searchWindow = OrthodromicDistance.suggestSearchWindow(point, maxDistanceInKm);
        GeoPipeline pipeline = start(tx, layer, new SearchIntersectWindow(layer, searchWindow)).calculateOrthodromicDistance(point);
     ...
     * @return geoPipeline
     */
    public static GeoPipeline startNearestNeighborSearch(final Transaction tx, Layer layer, Coordinate point, int numberOfItemsToFind) {
        Envelope searchWindow = SpatialTopologyUtils.createEnvelopeForGeometryDensityEstimate(tx, layer, point, numberOfItemsToFind);
     ...
     * @return geoPipeline
     */
    public static GeoPipeline startNearestNeighborSearch(final Transaction tx, Layer layer, Coordinate point, Envelope searchWindow) {
        return start(tx, layer, new SearchIntersectWindow(layer, searchWindow)).calculateDistance(layer.getGeometryFactory().createPoint(point));
    }
     ...
     * @return geoPipeline
     */
    public static GeoPipeline startNearestNeighborSearch(final Transaction tx, Layer layer, Coordinate point, double maxDistance) {
    ...
    public FeatureCollection<SimpleFeatureType, SimpleFeature> toStreamingFeatureCollection(final Transaction tx, final Envelope bounds) {
        return toStreamingFeatureCollection(tx, Neo4jFeatureBuilder.getTypeFromLayer(tx, layer), bounds);
    }

    public FeatureCollection<SimpleFeatureType, SimpleFeature> toStreamingFeatureCollection(final Transaction tx, SimpleFeatureType featureType, final Envelope bounds) {
    ...
    public FeatureCollection<SimpleFeatureType, SimpleFeature> toFeatureCollection(final Transaction tx, SimpleFeatureType featureType) {
