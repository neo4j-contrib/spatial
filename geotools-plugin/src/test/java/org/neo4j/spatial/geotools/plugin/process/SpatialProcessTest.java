...
package org.neo4j.spatial.geotools.plugin.process;
...
class SpatialProcessTest {
	...
	public void testProcess() throws ParseException, InterruptedException, ExecutionException {
		...
		try (ProcessExecutor engine = Processors.newProcessExecutor(2)) {

			// quick map of inputs
			Map<String, Object> input = new KVP("geom", geom);
			Progress working = engine.submit(process, input);

			// you could do other stuff while working is doing its thing
			...
			Assertions.assertEquals("POLYGON ((1 1, 1 3, 7 9, 7 6, 2 1, 1 1))", octo.toText());
		}
	}
}
